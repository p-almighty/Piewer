"""リファレンス超解像プラグイン: HTTPコネクタ（着色 connector の双子）。

純Python（標準ライブラリの urllib）＋ PIL だけで動く＝本体に重い依存を増やさない。
画像をHTTPで超解像サーバへ送り、高解像度化した画像を受け取って返す。送信先URLを
  ・http://127.0.0.1:7861/upscale にすればローカル（オフライン・無料・画像は自PC内）
...
着色 connector との違いは「colorize → upscale」だけ。拡大率・ノイズ除去はサーバ起動時に
決まる（どの重みを読むか）ため、ここでは画像を送るだけ。
"""
import io
import re
import json
...
class HttpUpscaler:
    ...
    name = "HTTP超解像コネクタ"
    version = "1.0"
    description = ("画像をHTTPで超解像サーバへ送り、高解像度化した結果を受け取ります。"
                   "URLを localhost にすればオフライン・無料、クラウドAPIにも繋げます。")

    def config_fields(self):
        return [
            {"key": "endpoint", "label": "エンドポイントURL", "type": "text", "default": "",
             "help": "例) http://127.0.0.1:7861/upscale （ローカル） / https://api.example.com/v1/upscale"},
            ...
            {"key": "max_side", "label": "送信前の最大辺(px)", "type": "int", "default": 0,
             "help": "大きすぎる画像は縮小してから送る。超解像では通常0（縮小しない）。"},
            {"key": "timeout", "label": "タイムアウト(秒)", "type": "int", "default": 180,
             "help": "1ページの高解像度化を待つ最大秒数（超解像はやや時間がかかる）。"},
        ]

    def available(self):
        # 設定はページごとに opts で渡るため、ここでは常に「使える」。
        # 実際の接続失敗は upscale 時に例外として表面化する。
        return (True, "")

    # ── 本体 ───────────────────────────────────────────────

    def upscale(self, img: "Image.Image", opts: dict) -> "Image.Image":
        endpoint = str((opts or {}).get("endpoint", "")).strip()
        if not endpoint:
            raise ValueError("エンドポイントURLが未設定です（AI超解像の設定から入力してください）")
        max_side = int((opts or {}).get("max_side", 0) or 0)
        timeout = int((opts or {}).get("timeout", 180) or 180)
        ...
        try:
            ...
            try:
                ...
                pass
            raise RuntimeError(f"超解像サーバがエラーを返しました (HTTP {e.code}) {detail}") from None
        except urllib.error.URLError as e:
            raise RuntimeError(f"超解像サーバに接続できません: {e.reason}") from None
